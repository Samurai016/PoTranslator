...
module.exports = {
  ...
  askInputLanguage: (translationModels) => {
    const inputLanguages = translationModels.map(lang => lang.source);
  ...
  askTranslation: (translation, translated, totalItems, index) => {
    const questions = [
      {
        name: 'translation',
        type: 'input',
        message: `[${index}/${totalItems}] Enter translation of the string (${translation}):`,
        default: translated,
        validate: function(value) {
          if (value) {
            return true;
